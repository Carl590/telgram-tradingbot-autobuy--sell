...
const startTradeBot = () => {
  ...
  bot.on("callback_query", (callbackQuery: TelegramBot.CallbackQuery) => {
    void callbackQueryHandler(bot, callbackQuery);
  });

  // bot message
  bot.on("message", (msg: TelegramBot.Message) => {
    void messageHandler(bot, msg);
  });

  // bot commands
  bot.onText(/\/start/, (msg: TelegramBot.Message) => {
    void handleStartCommand(bot, msg);
  });
  bot.onText(/\/position/, (msg: TelegramBot.Message) => {
    void positionScreenHandler(bot, msg);
  });

  bot.onText(/\/settings/, (msg: TelegramBot.Message) => {
    void settingScreenHandler(bot, msg);
  });

  alertBot.onText(/\/start/, (msg: TelegramBot.Message) => {
    void handleAlertBotStart(msg);
  });
  alertBot.on("new_chat_members", (msg: TelegramBot.Message) => {
    void handleNewChatMembers(msg);
  });
  alertBot.on("left_chat_member", (msg: TelegramBot.Message) => {
    void removeReferralChannelHandler(msg);
  });


const handleStartCommand = async (bot: TelegramBot, msg: TelegramBot.Message) => {
...
const handleAlertBotStart = async (msg: TelegramBot.Message) => {
...
const handleNewChatMembers = async (msg: TelegramBot.Message) => {
